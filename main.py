...
def analyze_match(message):
    ...
    bot.send_message(message.chat.id, f"🔍 Аналізую матч: *{user_input}*...", parse_mode="Markdown")
    ...
    ІНСТРУКЦІЯ:
    1. Якщо користувач вказав дату через слеш '/' (наприклад: Венеція - Фіорентина / 11.09.26), використай САМЕ ЦЮ ДАТУ для відповіді.
    2. Перший рядок відповіді БЕЗ ВИНЯТКІВ має відповідати формату:
    [Назва Команди 1] - [Назва Команди 2] / [Вказана Дата]

    3. Надай СТРOГО 1 або максимум 2 найвірогідніші ставки з коротким обґрунтуванням. Без таблиць, без HTML-тегів.
    ...
    • **Ймовірність:** 65%
    • **Обґрунтування:** Фіорентина в середньому здійснює 5.5 кутових у виїзних матчах даної серії.

    📌 **Тотал голів**
    • **Ставка:** Менше 2.5
    • **Ймовірність:** 60%
    • **Обґрунтування:** Команди демонструють низьку результативність у нинішньому відрізку сезону.
    """
    
    try:
        chat_completion = client.chat.completions.create(
            messages=[
                {"role": "system", "content": "Ти професійний спортивний аналітик. Точно дотримуйся дати користувача та заданого формату."},
